...
//在记录页面弹出时间对话框
class SelectTimeDialog: Dialog{
    lateinit var hourEt: EditText
    lateinit var minuteEt: EditText
    lateinit var datePicker: DatePicker
    lateinit var ensureBtn: Button
    lateinit var cancelBtn: Button
    private var onsureListener: ((String, Int, Int, Int) -> Unit)? = null

    fun setOnsureListener(onsureListener: ((String, Int, Int, Int) -> Unit)) {
...
//    }
    ...
    fun onClick() {
        cancelBtn.setOnClickListener { cancel() }  //点击取消按钮，取消对话框
        ensureBtn.setOnClickListener {
            ...
            onsureListener?.invoke(timeFormat, year, month, day)
            cancel()
        }
    }
